...
module.exports = async (client, msg, args) => {
  const usersString = args.join(' ');
  const users = {};

  const groups = /^(.*?)\sv\s(.*?)$/gi.exec(usersString);
  ...
  if (erroredSummoners.length > 0) {
    await msg.channel.send('Unable to retrieve summoner information');
    return;
  }
  ...
  if (erroredMatches.length > 0) {
    await msg.channel.send(JSON.stringify(userObjs));
    return;
  }
  ...
  if (erroredMatchObjResponses.length > 0) {
    await msg.channel.send('Error retrieving match information');
    return;
  }
  ...
  await msg.channel.send(stringBuilder);
};
